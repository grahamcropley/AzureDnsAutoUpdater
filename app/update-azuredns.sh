...
while IFS= read -r domain; do
  ...
  if [ ! -z $domain ]; then
    echo -n "${NORMAL}Processing: ${INFO}$domain"
    ...
    if [ $? -ne 0 ]; then
      ...
      if [ $? -eq 0 ]; then
        echo -ne "${NORMAL} CNAME ${OK}created${NORMAL}\n"
      else
        echo -ne "${NORMAL} CNAME ${ERR}failed${NORMAL}\n"
      fi
    else
      echo -ne "${NORMAL} CNAME ${WARN}already exists${NORMAL}\n"
    fi
  fi
done < "$zoneFile"
